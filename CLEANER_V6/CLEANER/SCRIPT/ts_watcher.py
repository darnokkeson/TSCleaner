...
CHECK_INTERVAL = 1        # seconds between folder scans
SIZE_CHECK_INTERVAL = 1   # seconds between file size checks
...
def wait_for_stable_size(filepath):
    ...
    while True:
        ...
        if i == 0:
            print("  [OCZEKIWANIE] Wykryto nowy plik. Czekam na pełny export.")
            i += 1
        if size1 == size2:
            return
...
def main():
    ...
    print("  ===============================================")
    print("  Nacisnij Ctrl+C aby zatrzymac, lub zamknij okno")
    print("  ===============================================")
    print()
    ...
    while True:
        ...
        for fname in os.listdir(root_dir):
            ...
            clean_fname = f"{clean_name}_clean{ext}"
            clean_path = os.path.join(root_dir, clean_fname)
            if os.path.exists(clean_path):
                continue  # Skip if clean version already exists
        ...
        if not new_files:
            if not waiting_shown:
                print(f"  [OCZEKIWANIE] Brak nowych plikow .ts. Sprawdzanie co {SIZE_CHECK_INTERVAL} sekund...")
                waiting_shown = True
        ...
        for fname, full_path in new_files:
            ...
            print(f"")
            print(f"  PLIK W PROCESIE, NIE ZAMYKAJ OKNA")
            print(f"  ------------------------------------------")
